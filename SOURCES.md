...
TagForge 官方数据集是经过归一化、重新分类和语义审核的设计词汇与原创命题集合，不是外部平台数据库的镜像。外部页面、标签和候选文本一律按不可信数据处理。
...
普通 Entry 的 family、facet、权重、风险和复合结构由维护者按游戏设计语义审核。官方数据集不维护人工 Relation。
...
2026.07.3 的历史更新是既有正式内容的确定性格式变更，没有新增外部候选：
