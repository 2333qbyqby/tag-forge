...
- Tag、Prompt、翻译、分类、语义家族、Facet、Recipe 和数据更新日期
- 官方 Pack、模板与预计算分析产物
...
脚本负责抓取和保存快照、字符串标准化、精确去重、ID 与引用校验、格式变更、Pack 规范化、哈希计算、Recipe 可达性检查、模拟、分析产物生成、测试和构建。
...
3. 运行 `pnpm data:report`，记录更新前 Entry、Prompt、Recipe 和数据更新日期基线。
4. 明确本轮是纯格式变更还是内容更新。
...
审核记录至少包含：协议修订日期、前后数据更新日期、范围、来源状态、格式变更守恒、AI 语义工作、脚本工作、Category/Recipe 变化、分析结果、模拟结果、验证结果和已知限制。
...
- [ ] Pack ID、Entry ID 和 Prompt ID 完整。
