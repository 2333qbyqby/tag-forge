...
data-src
  ...
  → canonical DataPack
  → generation logic + React views + IndexedDB
```
...
`src/packs/types.ts` 定义当前 Data Pack Draft 和结果快照。`validate.ts` 只接受声明式 Recipe；`importer.ts` 将 JSON 与 ZIP/CSV 归一成相同对象；`canonical.ts` 负责稳定 JSON 和 SHA-256；`compile.ts` 建立运行时索引。

官方能力不信任数据包声明。`src/packs/official.ts` 读取构建生成的 checksum 注册表，只有注册表匹配的官方包才获得 `analysis: true`。用户安装包和临时包始终没有高级分析权限。

## 生成器

`src/engine/pack-engine.ts` 不依赖 React、DOM、IndexedDB 或网络。它只读取 `CompiledPack`、设置和历史快照，并返回 `ResultSnapshot`。
...
官方包来自静态文件，不复制到 IndexedDB。临时包只在 React 内存状态中存在。开发期数据库名为 `tagforge-dev`；不读取或迁移旧浏览器数据。
...
链接在 URL Fragment 中携带当前结果快照，包括 pack 引用、Recipe、Seed 和槽位文本。缺少外部包时仍能显示文本，但结果只读。旧 query 链接不再解析。
